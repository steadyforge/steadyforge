...
sudo chmod 777 /home/pi/steadyforge/rotate/retroarch.cfg
sudo cp /home/pi/steadyforge/rotate/retroarch.cfg /opt/retropie/configs/$3/retroarch.cfg
sudo chmod 777 /opt/retropie/configs/$3/retroarch.cfg
function get_config() {
    declare -Ag MODE_MAP
...
function choose_emulator() {
    ...
    if [[ -z "${options[*]}" ]]; then
        dialog --msgbox "No emulator options found for $SYSTEM - Do you have a valid $EMU_SYS_CONF ?" 20 60 >/dev/tty1
        stop_joy2key
        exit 1
    fi
...
function show_launch() {
    ...
    if [[ -n "$image" ]]; then
        ...
        if [[ -n "$DISPLAY" ]]; then
            ...
            fbi -1 -t "$IMAGE_DELAY" -noverbose -a "$image" </dev/tty1 &>/dev/null
        fi
...
function check_menu() {
    ...
    IFS= read -s -t 2 -N 1 key </dev/tty1
    if [[ -n "$key" ]]; then
...
function user_script() {
    ...
    if [[ -f "$script" ]]; then
        bash "$script" "$SYSTEM" "$EMULATOR" "$ROM" "$COMMAND" </dev/tty1 2>>"$LOG"
    fi
}
...
function launch_command() {
    ...
    if [[ "$CONSOLE_OUT" -eq 1 ]]; then
        # turn cursor on
        tput cnorm
        eval $COMMAND </dev/tty1 2>>"$LOG"
        ret=$?
        tput civis
    else
        eval $COMMAND </dev/tty1 &>>"$LOG"
        ret=$?
    fi
    return $ret
}
